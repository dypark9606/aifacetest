...
**속도**: 12코어 맥에서 **초당 500장** (161명 2373장이 4.7초). `--jobs` 로 조절.

**주의**: 사람마다 `pool.map` 을 반복하면 spawn+PyObjC 조합에서 멎는다(2026-09-07 실측).
그래서 이 도구는 **모든 사진을 한 번에 모아 한 번만** 병렬 처리한다. 고치지 말 것.
...
- **개그맨 폴더는 클래스명이 `개그맨강석` 형태**라 `--strip-token 얼굴` 만으로는 안 되고
  **`--strip-prefix 개그맨` 을 같이 줘야** 라벨이 모델과 맞는다. 안 그러면 화면에
  '개그맨강석' 이 그대로 뜬다.
- **품질검사를 통과한 사진이 15장 미만인 인물이 337명**이다. 전체 510명의 66%.
  (남연예인 109/161 · 여연예인 82/160 · 개그맨 106/136 · 개그우먼 40/53)
  `retrain.py` 기준(원본 10장 미만)으로는 75명이지만, 흐림·중복을 걸러내면 이만큼 준다.
